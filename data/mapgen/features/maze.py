...
## Carve a straight tunnel.
def carveTunnel(map, sector):
    straight.carveTunnel(map, sector)
    

## @package maze Use a growing tree algorithm:
# http://www.astrolog.org/labyrnth/algrithm.htm
# to create the maze.
def createFeature(map, sector):
    # Fill our area with a maze, putting an opening at either end.
    (startLoc, endLoc) = sector.getStartAndEndLoc()
    if startLoc is None or endLoc is None:
        print "Unable to make maze for",sector.id,"from",sector.loc,"to",sector.parent.loc
        sector.featureData['madeMaze'] = 0
        return

    (minX, minY, maxX, maxY) = sector.getSectorBounds()
    ...
    while cellStack:
        ...
        for i in range(1, mazeMaximumColumnHeight):
            ...
            # outside the sector. Assume that platforms will fix things once
            # we're out.
            ...
            if (offY >= map.numRows or 
                    map.blocks[cell[0]][offY] != 0 or
                    (cell[0], offY) not in sector.spaces):
                break
            verticalClearance += 1
        for i in range(1, mazeMaximumColumnHeight):
            offY = cell[1] - i
        ...
        for offset in constants.NEWSPerimeterOrder:
            ...
            x = cell[0] + offset[0] * 2
            y = cell[1] + offset[1] * 2
            if (x, y) in sector.spaces and (x, y) not in seenCells:
        ...
        map.blocks[cell[0] + dx][cell[1] + dy] = 0
    ...
    shouldRevert = 0
    for loc in sector.spaces:
        if loc not in seenCells:
            shouldRevert = 1
            break
    if shouldRevert:
        for block in changedBlocks:
            map.blocks[block[0]][block[1]] = 0
        sector.featureData['madeMaze'] = 0
